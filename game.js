...
// ==========================================
// RASTREAMENTO DE PROGRESSO DA INVESTIGAÇÃO
//
// Para tornar o jogo mais desafiador e aproximá-lo da realidade,
// acompanhamos quais provas e testemunhas já foram analisadas.
// O jogador não pode acusar um suspeito até que todas as provas
// tenham sido vistas e todas as testemunhas interrogadas. Estes
// arrays são redefinidos sempre que um novo caso é carregado.
let viewedEvidence = [];
let viewedWitnesses = [];
...
// AUTOSAVE SYSTEM
// ===========================
// Track the last screen visited so we can restore it on reload.
let lastScreen = "introScreen";

/**
 * Persists the current game state to localStorage.  The data saved
 * includes the player name, selected avatar, progress through the
 * case list, the current stats object and the last screen shown.
 */
function saveGame() {
  try {
    const data = {
    ...
    // Se o armazenamento local não estiver disponível, ignora erros.
    console.error("Erro ao salvar o jogo:", err);
  }
}

/**
 * Restaura o jogo salvo do localStorage, se existir. Esta função
 * recupera os valores do avatar, nome do jogador, progresso e
 * estatísticas. Qualquer campo ausente no save será ignorado.
 */
function loadGame() {
  try {
    ...
    if (data.selectedAvatar) selectedAvatar = data.selectedAvatar;
    if (data.playerName) playerName = data.playerName;
    if (typeof data.currentCaseIndex === "number") currentCaseIndex = data.currentCaseIndex;
...
if (typeof document !== 'undefined') {
  // No carregamento do DOM, tentamos carregar um jogo salvo.  Se
  // existir um nome de jogador salvo, pulamos a introdução e
...
function show(screenId) {
  ...
  // Registra a tela atual para fins de salvamento automático.
  lastScreen = screenId;
  // Salva o jogo sempre que uma nova tela é exibida para garantir que o
  // progresso seja persistido.  Isso captura mudanças de tela como
  // conclusão de casos e promoções.
  saveGame();
}
...
function updateHUD() {
  ...
  // Após atualizar o HUD e ajustar as estatísticas, persista o estado
  // atual.  Isso garante que alterações nos pontos, prestígio ou
  // reputação sejam gravadas imediatamente.
  saveGame();
}
...
function endExam() {
  ...
  // Determina a nota mínima necessária com base na agência selecionada (CIA exige 80%, outras 70%)
  const requiredPercent = selectedAgency === "CIA" ? 80 : 70;
  if (percent >= requiredPercent) {
    // Aprovado
    gameState.agency = selectedAgency;
    if (resultEl) {
      // Construir mensagem de aprovação em duas partes para evitar quebras de linha no código
      const passMessage = `Parabéns! Você passou no exame com ${percent.toFixed(0)}% e atingiu a nota mínima de ${requiredPercent}%.`;
      const roleMessage = `Agora você é ${selectedAgency.toUpperCase()}. Continue investigando casos mais complexos em sua nova função.`;
      resultEl.textContent = passMessage + "\n" + roleMessage;
    }
  } else {
    if (resultEl) {
      resultEl.textContent = `Você obteve ${percent.toFixed(0)}% e não atingiu os ${requiredPercent}% necessários. Tente novamente após resolver mais casos.`;
    }
  }
  // Resetar seleção
  selectedAgency = "";
...
async function loadNextCase() {
  ...
  // Reinicia o progresso de provas e testemunhas para o novo caso
  resetCaseProgress();
...
function openInvestigation() {
  ...
  // Atualiza a barra de progresso no início da investigação
...
function chooseSuspect(fileName) {
  ...
  // Exige que todas as provas e testemunhas tenham sido analisadas antes de acusar
  const totalEvidence = currentCase.evidence ? currentCase.evidence.length : 0;
  const totalWitnesses = currentCase.witnesses ? currentCase.witnesses.length : 0;
  if (viewedEvidence.length < totalEvidence || viewedWitnesses.length < totalWitnesses) {
    alert("Você ainda não analisou todas as provas e interrogou todas as testemunhas. Continue investigando antes de acusar.");
    return;
  }
...
function randomEvent() {
  const events = [
    ...
    ,
    // Eventos adicionais para refletir procedimentos investigativos e legais
    {
      msg: "Você ignorou a cadeia de custódia e perdeu evidências, afetando sua reputação.",
      effect: () => {
        gameState.reputation = Math.max(0, gameState.reputation - 5);
      }
    },
    {
      msg: "Você obteve um mandado e descobriu novas evidências, aumentando seu prestígio.",
      effect: () => {
        gameState.prestige = Math.min(100, gameState.prestige + 5);
      }
    },
    {
      msg: "Seu pedido de mandado foi negado e o suspeito fugiu, reduzindo sua moral.",
      effect: () => {
        gameState.moral = Math.max(0, gameState.moral - 5);
      }
    }
...
// ===========================================
// PROGRESSO DE PROVAS E TESTEMUNHAS
//
// Estas funções controlam o registro de quais provas e
// testemunhas já foram analisadas durante um caso. Quando um
// novo caso é carregado, chame resetCaseProgress() para
// limpar os registros. Durante a investigação, chame
// registerEvidenceViewed() ou registerWitnessViewed() ao abrir
// detalhes de uma prova ou iniciar o interrogatório de uma
// testemunha. A função updateInvestigationProgress() atualiza
// o texto de progresso na tela de investigação.
...
function updateInvestigationProgress() {
  const progressEl = document.getElementById("investigationProgress");
  if (!progressEl || !currentCase) return;
  const totalEvidence = currentCase.evidence ? currentCase.evidence.length : 0;
  const totalWitnesses = currentCase.witnesses ? currentCase.witnesses.length : 0;
  progressEl.textContent = `Progresso: ${viewedEvidence.length}/${totalEvidence} provas analisadas, ${viewedWitnesses.length}/${totalWitnesses} testemunhas interrogadas.`;
}

function registerEvidenceViewed(fileName) {
...
function registerWitnessViewed(fileName) {
...
function startExam() {
  ...
  if (selectedAgency === "FBI") {
    // Exame para o FBI: perguntas abordam procedimentos investigativos
    examQuestions = [
      ...
      // Novas questões focadas em procedimentos e leis
      { q: "Ao chegar em uma cena de crime, qual deve ser a primeira ação do detetive?", options: ["Preservar a segurança e isolar a área", "Interrogar suspeitos imediatamente", "Coletar provas sem documentação"], correct: 0 },
      { q: "O que diz o princípio de Locard?", options: ["É um protocolo de investigações cibernéticas", "Todo contato deixa traços de evidência", "Uma lei que regula interrogatórios"], correct: 1 },
      { q: "Quem pode convocar um Grand Jury nos EUA?", options: ["Promotores federais", "Juízes de paz", "Agentes de segurança privada"], correct: 0 },
      { q: "Quais crimes são tipicamente de competência federal?", options: ["Homicídios locais", "Crimes interestaduais e federais", "Infrações de trânsito"], correct: 1 },
      { q: "A cadeia de custódia garante que...", options: ["Qualquer pessoa pode manipular as provas", "As evidências sejam documentadas e preservadas", "As provas sejam descartadas após uso"], correct: 1 }
    ];
  } else if (selectedAgency === "CIA") {
    // Exame para a CIA: perguntas sobre inteligência e operações internacionais
    examQuestions = [
      ...
      // Novas questões focadas no ciclo de inteligência
      { q: "Qual etapa inicia o ciclo de inteligência?", options: ["Coleta", "Planejamento e requisitos", "Análise"], correct: 1 },
      { q: "Qual destes métodos de coleta NÃO é utilizado pela CIA?", options: ["Sinais (SIGINT)", "Otimização de tráfego", "Fonte aberta (OSINT)"], correct: 1 },
      { q: "Qual é a finalidade da análise de inteligência?", options: ["Criar relatórios para tomadores de decisão", "Realizar investigações locais de crimes menores", "Gerenciar orçamentos de agências"], correct: 0 },
      { q: "Após a disseminação de um relatório, qual etapa ocorre no ciclo de inteligência?", options: ["Avaliação e feedback", "Coleta", "Processamento"], correct: 0 },
      { q: "A CIA pode legalmente...", options: ["Conduzir operações domésticas sem supervisão", "Participar de monitoramento internacional autorizado", "Realizar prisões dentro dos EUA"], correct: 1 }
    ];
  } else {
    examQuestions = [];
  }
...
function openDetail(fileName) {
  ...
  // Registra que esta prova foi visualizada (se aplicável)
...
function openWitnessDetail(fileName) {
  ...
  // Registra que esta testemunha foi interrogada
  registerWitnessViewed(fileName);
  witnessMood = 100;
